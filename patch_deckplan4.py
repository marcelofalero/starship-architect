    ...
    setup() {
        ...
        const bringForward = () => {
            ...
            selectedItemIds.value.forEach(id => {
                const item = activeSection.value.items.find(i => i.id === id);
                if (item) {
                    if (item.z === undefined) item.z = getDefaultZ(item.type);
                    item.z += 1;
                }
            });
        };
        ...
        return {
            ...
            bringForward, sendBackward, elementsTab, sortedSectionItems, toggleVisibility
        };
    },
    template: `
        ...
        <div class="col-12 col-md-3 q-pr-md flex column">
            ...
            <q-card dark class="bg-grey-9 q-mb-md">
                <q-card-section>
                    ...
                    <div v-if="!showMasterMap" class="q-mt-sm row q-col-gutter-sm">
                        ...
                        <div class="col-6"><q-btn :disable="selectedItemIds.length===0" color="secondary" outline size="sm" class="full-width" @click="bringForward">Bring Forward</q-btn></div>
                        <div class="col-6"><q-btn :disable="selectedItemIds.length===0" color="secondary" outline size="sm" class="full-width" @click="sendBackward">Send Backward</q-btn></div>
                    </div>
                </q-card-section>
            </q-card>
            ...
            <q-card dark class="bg-grey-9 col flex column" v-if="!showMasterMap">
                ...
                <q-card-section class="col" style="overflow-y: auto;" v-if="elementsTab === 'systems'">
                    ...
                    <div v-for="c in sectionComponents" :key="c.instanceId" 
        ...
        <div class="col-12 col-md-9" style="overflow: auto; background-color: #000; border: 2px solid #4BB5C1; position: relative;" @mouseleave="onGridMouseUp" @mouseup="onGridMouseUp">
            ...
            <div v-else-if="activeSection" 
                 ...
                 <div v-for="item in activeSection.items" :key="item.id" 
                     ...
                     <div v-if="selectedItemIds.length === 1 && selectedItemIds.includes(item.id) && activeTool === 'select' && !item.hidden" 
