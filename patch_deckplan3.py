...
export const DeckPlanWrapper = {
    setup() {
        ...
        const copySection = () => {
            if (!activeSection.value) return;
            const toCopy = activeSection.value.items.filter(i => i.type !== 'component');
        ...
        const pasteSection = (mirrorX = false, mirrorY = false) => {
            ...
            const newItems = clipboard.value.map(item => {
                ...
                if (mirrorX) {
                    cloned.x = secW - cloned.w - cloned.x;
                }
                if (mirrorY) {
                    cloned.y = secH - cloned.h - cloned.y;
                }
        ...
        const onGridMouseUp = () => {
            ...
            if (interaction.value.mode === 'marquee' && selectionBox.value) {
                ...
                selectedItemIds.value = selected.map(i => i.id);
                selectionBox.value = null;
            }
        ...
        return {
            ...
            clipboard, copySection, pasteSection, clearSection
        };
    },
    template: `
                ...
                <q-card-section>
                    ...
                    <div v-if="activeSection" class="q-mt-sm q-pa-sm bg-dark rounded-borders">
                        ...
                        <div class="row q-col-gutter-sm">
                            ...
                            <div class="col-6"><q-btn outline color="negative" size="sm" class="full-width" icon="delete_sweep" @click="clearSection">Clear</q-btn></div>
                            <div class="col-12"><q-btn outline :disable="clipboard.length===0" color="secondary" size="sm" class="full-width q-mt-xs" icon="content_paste" @click="pasteSection(false, false)">Paste</q-btn></div>
                            <div class="col-6"><q-btn outline :disable="clipboard.length===0" color="accent" size="sm" class="full-width q-mt-xs" icon="flip" @click="pasteSection(true, false)">Paste Flip X</q-btn></div>
                            <div class="col-6"><q-btn outline :disable="clipboard.length===0" color="accent" size="sm" class="full-width q-mt-xs" icon="flip_camera_android" @click="pasteSection(false, true)">Paste Flip Y</q-btn></div>
                        </div>
                    </div>
                </q-card-section>
            </q-card>
        ...
        <div class="col-12 col-md-9" style="overflow: auto; background-color: #000; border: 2px solid #4BB5C1; position: relative;" @mouseleave="onGridMouseUp" @mouseup="onGridMouseUp">
            ...
            <div v-else-if="activeSection" 
                 ...
                 <div v-for="item in activeSection.items" :key="item.id" 
                     ...
                     <div v-if="selectedItemIds.length === 1 && selectedItemIds.includes(item.id) && activeTool === 'select'" 
